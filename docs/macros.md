...
an iteration counter for a macro `DO` loop. These global argument definitions are illustrated in the examples below:
```no-highlight
...
to prompt for values while providing a default. Examples of local argument definitions and assignments are shown below:
```no-highlight
